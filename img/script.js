...
document.getElementById("cancelBeforeStartBtn").onclick = async () => {
  if (!isAdmin) return;

  const confirmDelete = confirm("Batalkan sesi dan hapus semua peserta?");
  ...
  alert("Sesi dibatalkan. Semua peserta dihapus.");
};

document.getElementById("endSessionBtn").onclick = async () => {
  if (!isAdmin) return;

  await db.collection("session").doc("current").update({
    ended: true
  });
...
function setupSessionListener() {
  db.collection("session").doc("current").onSnapshot(doc => {
    ...
    countdown = getRemainingSeconds(data.startedAt?.toDate?.() || new Date(), currentRound);

    if (data.ended) {
      clearInterval(timerInterval);
      document.getElementById("timer").innerText = "Sesi berakhir.";
      document.getElementById("startBtn").style.display = "none";
      document.getElementById("cancelBeforeStartBtn").style.display = "none";
      document.getElementById("newSessionBtn").style.display = "block";
      document.getElementById("endSessionBtn").style.display = "none";
      return;
    } else {
      document.getElementById("cancelBeforeStartBtn").style.display = "inline-block";
...
function updateTimerUI() {
  ...
  document.getElementById("timer").innerText = "Sisa waktu: " + min + "m " + sec + "s";
}
